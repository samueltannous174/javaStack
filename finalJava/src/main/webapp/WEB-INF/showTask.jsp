...
<div class="  p-6 bg-blue-50 shadow rounded  mx-auto w-3/4">
    ...
    <div class="flex justify-between items-start">
        ...
        <div class="flex flex-col gap-3">
            <p class="font-bold text-black-600">${task.moreDetails}</p>
            <p class="font-bold text-purple-600"> your deadline is ${task.deadLine} </p>
        </div>
    </div>

    <c:if test="${logged.id == task.user.id}">
...
<div>
    <c:forEach var="tag" items="${task.tags}">
        <span class="px-2 py-1 bg-gray-200 text-gray-800 rounded">${tag.name}</span>
    </c:forEach>
</div>


<form action="/change_type" method="post">
    <input type="hidden" name="id" value="${task.id}">
    <input type="hidden" name="type" value="InProgress">
    <button class="px-4 ml-10 py-2 bg-purple-600 text-white rounded hover:bg-red-700">
        InProgress
    </button>
</form>
...
<form action="/change_type" method="post">
    ...
    <button class="ml-10 mt-2 px-4 py-2 bg-purple-600 text-white rounded hover:bg-red-700">
        Done
    </button>
</form>
...
<div class="mt-10 text-center">
    <a href="/dashboard?type=all" class="inline-block px-10 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold rounded-full shadow-lg transform transition hover:scale-105 hover:from-blue-600 hover:to-indigo-700 duration-300">
        Back to List
    </a>
</div>
